...
import {
  ...
  TrendingUp, Users, Download, Award, QrCode, Filter
} from 'lucide-react';
import { api } from '../../api';
...
export default function AdminStudents() {
  ...
  const deleteStudent = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-students'] }); showToast('✅ Data siswa dihapus'); },
    ...
    <motion.div variants={pageVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          ['Aktif', list.filter(s => s.status !== 'inactive').length, '#00B894', TrendingUp],
          ['Rata Kehadiran', '—%', '#E67E22', Award],
          ['Kelas', classList.length, '#2E2BBF', Users],
      ...
      <motion.div variants={cardVariants} className="retro-card bg-base-white border-4 border-base-black p-4 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="Cari nama atau NIS..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            ...
            <tbody>
              {isLoading ? (
                ...
                <motion.tr key={student.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="p-4 font-retro-mono text-xs text-base-black/60">{student.nis || '—'}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => { if (confirm('Hapus siswa ini?')) deleteStudent.mutate(student.id); }}
                ...
                <tr><td colSpan="7" className="p-10 text-center">
                  ...
                  <p className="font-retro-mono text-base-black/50">Tidak ada data siswa</p>
                </td></tr>
              )}
            </tbody>
          </table>
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={`👤 ${selected?.name}`} maxWidth="md">
        {selected && (
